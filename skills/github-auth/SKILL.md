...
| `curl` GitHub API returns 401 with `Authorization: token <PAT>` | GitHub API auth can fail with the Bearer-style header in some environments. Use `-u "<PAT>"` instead (Basic auth). Also add `-L` so curl follows redirects. Example: `curl -sL -u "ghp_..." "https://api.github.com/repos/OWNER/REPO/actions/runs?per_page=1"`. |
